...
module.exports = function (grunt) {
	...
	grunt.registerMultiTask('min', 'Minify files with UglifyJS.', function () {
		...
		var banner = grunt.task.directive(files[0], function () {
				return null;
			});
		if (banner === null) {
			banner = '';
		...
		if(!!this.file.dest){
			...
			files.map(function(filepath){
				var max= grunt.task.directive(filepath, grunt.file.read);
				var min= banner+ grunt.helper('uglify', max, grunt.config('uglify'));
				
				var pubPath= grunt.helper('getPubPath', filepath, '.pub');
				if(!pubPath) return;
				grunt.file.write(pubPath, min);
				grunt.helper('min_max_info', min, max);
				
				if(this.errorCount) return;
				grunt.log.writeln('File "'+ pubPath+ '" published');
			})
		}
	});
